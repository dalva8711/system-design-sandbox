...
function NodeInspectorSection({
  ...
  const d = coerceNodeData(node.data);
  const nid = node.id;
  ...
  const commitLabel = useCallback(() => {
    const n = useDesignStore.getState().nodes.find((x) => x.id === nid);
    if (!n) return;
    const committed = coerceNodeData(n.data).label;
    if (labelDraft !== committed) {
      updateNodeData(nid, { label: labelDraft });
    }
  }, [labelDraft, nid, updateNodeData]);
    ...
    <section className="space-y-3 rounded-lg border border-black/10 bg-black/[0.02] p-3 dark:border-white/10 dark:bg-white/[0.03]">
          ...
          {(
            ...
            >
              <input
                ...
                onChange={() =>
                  updateNodeData(node.id, { status: value })
                }
              />
              {label}
            </label>
      ...
      <NodeBehaviorFields
        ...
        onBehaviorChange={(next: NodeBehavior) =>
          updateNodeData(node.id, { behavior: next })
        }
      />
      {hasTickData ? (
